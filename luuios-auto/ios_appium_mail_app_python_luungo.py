...
if platform == "linux" or platform == "linux2":
    ...
    json_file = local + "/appium_mail_app_ios.json"
    with open(json_file) as json_data_file:
        data = json.load(json_data_file)
    log_folder = "/Log/"
    execution_log = local + log_folder + "mailapp_execution_log_" + str(objects.date_id) + ".txt"
...
app_path = 'Users/hanbiro/Desktop/luuios/MailApp.ipa'
...
def log_in_mail_app_ios():
    ...
    try:
        ...
        if  click_btn_create_new.is_displayed():
            Logging("=> No Crash App") 
            TestCase_LogResult(**data["testcase_result"]["mail_app"]["log_in_mail_app_ios"]["pass"])
        else:
            ValidateFailResultAndSystem("=> Crash App")
            TestCase_LogResult(**data["testcase_result"]["mail_app"]["log_in_mail_app_ios"]["fail"])
            exit(0)
    except WebDriverException:
        ValidateFailResultAndSystem("=> Crash App")
        exit(0)
